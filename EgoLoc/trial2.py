...
def process_task(credentials, video_path, action, grid_size, total_frames, search_anchor, speed_folder, frame_index=None, flag=None, pinch_folder=None, trial_idx=None, stage=None):
    """
    ...
    if frame_index is None:
        ...
        print(f'The frame indices in {video_name} used for the localization grid: {used_frame_indices}')
    else:
        if flag == "feedback":
            print(f"Activate feedback mechanism with visual cues")
            invalid_list, speed_selected = select_top_n_frames_from_json(json_path, 4, frame_index, flag, receive_flag="right")
            selected_indices = _merge_speed_and_pinch(speed_selected, _pinch_indices())
            ...
            print(f'Resampled frame indices in {video_name} used for the localization grid: {used_frame_indices}')
            image = create_frame_grid_with_keyframe(
                video_path, used_frame_indices, grid_size)
        elif flag == "speed":
            ...
            invalid_list, speed_selected = select_top_n_frames_from_json(json_path, 4, frame_index, receive_flag="right")
            selected_indices = _merge_speed_and_pinch(speed_selected, _pinch_indices())
            total_indices = select_top_n_frames_from_json(json_path, total_frames, frame_index, flag)
            filter_indices = select_and_filter_keyframes_with_anchor(selected_indices, total_indices, 4, search_anchor, video_path)
            if not filter_indices:
    ...
    if description is None or description == -1:
        return None
    ...
    annotated_image = create_frame_grid_with_keyframe(
        video_path, used_frame_indices, grid_size, selected_index=selected_frame_index)
    trial_folder = f"trial_{trial_idx:02d}" if trial_idx is not None else "trial_unknown"
    stage_label = _safe_filename_part(stage or _default_localization_stage(search_anchor, frame_index, flag))
