"""Final Trial-2 EgoLoc protocol used by the OccluBench sampling ablation.

This file intentionally leaves ``trial2.py`` and ``trial3.py`` untouched.
It retains Trial 2's average-centered consecutive-grid topology while adding
strict sampling modes, the exact task prompt, persistent Transformers backend
injection, deterministic seeds, and complete request tracing.
"""
...
def compute_gap_normalized_speed(centers_by_frame, total_frames):
    """Return px/frame speed, normalizing displacement across detection gaps."""
    speed_dict = {}
    prev_center = None
    prev_center_frame = None
    for idx in range(int(total_frames)):
        center = centers_by_frame.get(idx)
        if center is None:
            speed = 0.0
        elif prev_center is None:
            speed = 0.0
            prev_center = center
            prev_center_frame = idx
        else:
            frame_gap = max(1, idx - prev_center_frame)
            displacement = (
                (center[0] - prev_center[0]) ** 2
                + (center[1] - prev_center[1]) ** 2
            ) ** 0.5
            speed = float(displacement / frame_gap)
            prev_center = center
            prev_center_frame = idx
...
def extract_2d_speed_and_visualize(video_path, output_dir, detector, vitpose):
    ...
    cap.release()
    speed_dict = compute_gap_normalized_speed(
        all_bounding_box_centers, total_frames
    )
...
def _relative_minimum_frame_indices(detected_items):
    """
    Return local minima computed independently inside contiguous detections.

    Missing-hand gaps split the signal. Detections on opposite sides of an
    occlusion are never treated as temporal neighbors. Within each contiguous
    segment, Trial 2's original rule is preserved: use strict/interior local
    minima, falling back to that segment's global minimum when none exists.
    """
    ...
    for segment in segments:
        ...
        for i in range(1, len(segment) - 1):
            prev_distance = segment[i - 1][1]
            frame_idx, distance = segment[i]
            next_distance = segment[i + 1][1]
            if distance <= prev_distance and distance <= next_distance and (
                distance < prev_distance or distance < next_distance
            ):
                segment_minima.append(frame_idx)
        if not segment_minima:
            frame_idx, _ = min(segment, key=lambda item: item[1])
            segment_minima.append(frame_idx)
        minima.extend(segment_minima)
...
def feedback_contact(credentials, video_path, action, grid_size, total_frames, frame_start, max_feedbacks, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend=None):
    """
...
def feedback_separation(credentials, video_path, action, grid_size, total_frames, frame_end, max_feedbacks, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend=None):
    """
...
def convert_video(video_path, action, credentials, grid_size, speed_folder, max_feedbacks, repeat_times=5, pinch_folder=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend=None):
    """
...
def select_pinch_keyframes(pinch_json_path, percentile=None):
    ...
    The current JSON format stores one record per video fraarks
...
SAMPLING_MODES = ("speed", "pinch", "speed_pinch")


def validate_sampling_mode(mode):
    if mode not in SAMPLING_MODES:
        raise ValueError(
            f"mode must be one of {SAMPLING_MODES}, received {mode!r}"
        )
    return mode


def derive_seed(episode, task, trial):
    """Stable paired seed independent of sampling mode."""
    payload = f"{episode}|{task}|{int(trial)}".encode("utf-8")
    return int(hashlib.sha256(payload).hexdigest()[:8], 16)


def build_exact_localization_prompt(search_anchor, action):
    """Return the common exact-event prompt, retaining the ``-1`` option."""
    common = (
        "I will show a sequence of nine chronologically consecutive images. "
        "I have annotated the images with numbered circles. "
    )
    if search_anchor == "start":
        task = (
            f"Choose the number corresponding to the exact earliest frame at "
            f"which a stable grasp of the object is established for the "
            f"action ({action}). "
            "If no stable grasp is established in any displayed frame, "
            "choose the number -1. "
        )
    elif search_anchor == "end":
        task = (
            f"Choose the number corresponding to the exact earliest frame at "
            f"which visible release of the object occurs, ending the action "
            f"({action}). "
            "If no visible release occurs in any displayed frame, choose the "
            "number -1. "
        )
    else:
    ...
    return (
        common
        + task
        + "Give a one sentence analysis of why you chose that point "
        "(less than 50 words). Provide your answer at the end in JSON in "
        'this format: {"points": []}'
    )
...
def build_mode_candidates(
    ...
    """Construct Trial-2 cue centers without midpoint filtering or leakage."""
    ...
    if mode in ("speed", "speed_pinch"):
        if speed_json_path is None:
            raise FileNotFoundError("speed mode requires a speed JSON path")
        if frame_index is None:
            speed_candidates = select_top_n_frames_from_json(
                speed_json_path, n_candidates
            )
        else:
        ...
        total_pool = select_top_n_frames_from_json(
            speed_json_path,
            total_frames,
            frame_index,
            flag,
        ...
        total_pool = list(range(total_frames))

    if mode in ("pinch", "speed_pinch"):
        if pinch_json_path is None:
            pinch_candidates = []
        else:
            pinch_candidates = select_pinch_keyframes(pinch_json_path)
        if frame_index is not None and flag == "feedback":
            reference = int(frame_index)
            pinch_candidates = [
                value for value in pinch_candidates if value > reference
            ]
            if mode == "pinch":
                total_pool = list(range(reference + 1, total_frames))
        elif frame_index is not None and flag not in ("speed",):
            reference = int(frame_index)
            pinch_candidates = [
                value for value in pinch_candidates if value != reference
            ]
            if mode == "pinch":
                total_pool = [
                    value for value in total_pool if value != reference
                ]

    if mode == "speed":
        cue_candidates = _clean_int_list(speed_candidates)
    elif mode == "pinch":
        cue_candidates = _clean_int_list(pinch_candidates)
        invalid_list = []
    else:
        cue_candidates = _merge_speed_and_pinch(
            _clean_int_list(speed_candidates),
            _clean_int_list(pinch_candidates),
        )

    center_candidates = list(cue_candidates)
    fallback_frames = []
    for frame_idx in _clean_int_list(total_pool):
        if len(center_candidates) >= int(n_candidates):
            break
        if frame_idx not in center_candidates:
            center_candidates.append(frame_idx)
            fallback_frames.append(frame_idx)
    if not center_candidates:
        center_candidates = list(range(min(total_frames, int(n_candidates))))
        fallback_frames = list(center_candidates)

    return {
        "candidates": center_candidates,
        "cue_candidates": cue_candidates,
        "total_pool": _clean_int_list(total_pool),
        "fallback_frames": fallback_frames,
        "invalid_list": _clean_int_list(invalid_list),
        "candidate_sources": {
            "speed": _clean_int_list(speed_candidates),
            "pinch": _clean_int_list(pinch_candidates),
        },
        "signal_reads": {
...
def build_consecutive_candidate_grid(
    selected_indices,
    grid_size,
    total_frames,
    min_index=None,
):
    """Return one clamped, unique, consecutive Trial-2 window."""
    target = int(grid_size) ** 2
    total_frames = int(total_frames)
    if total_frames < target:
        raise ValueError(
            f"video has {total_frames} frames but grid requires {target}"
        )
    clean = [
        value
        for value in _clean_int_list(selected_indices)
        if 0 <= value < total_frames
    ]
    center = (
        int(round(float(np.mean(clean))))
        if clean
        else (total_frames - 1) // 2
    )
    ideal_start = max(0, min(center - target // 2, total_frames - target))
    starts = list(range(total_frames - target + 1))
    min_relaxed = False
    if min_index is not None:
        allowed = [start for start in starts if start > int(min_index)]
        if allowed:
            starts = allowed
        else:
            min_relaxed = True
    start = min(starts, key=lambda value: (abs(value - ideal_start), value))
    frames = list(range(start, start + target))
    return frames, {
        "cue_average": center,
        "ideal_start": ideal_start,
        "window_start": start,
        "window_end": start + target - 1,
        "min_index": int(min_index) if min_index is not None else None,
        "min_index_relaxed": min_relaxed,
...
def _default_backend():
    module_dir = os.path.join(ROOT, "ablation-tests")
    if module_dir not in sys.path:
        sys.path.insert(0, module_dir)
    from backend import get_shared_backend
...
def vlm_request(
    ...
    """Run one request through a persistent Transformers backend object."""
    backend = _default_backend() if backend is None else backend
    if not hasattr(backend, "generate"):
        raise TypeError("inference_backend must provide generate(...)")
    ...
    try:
        ...
        if trace_out is not None:
            trace_out["latency_s"] = time.monotonic() - started
            trace_out["backend_used"] = str(
    ...
    if isinstance(result, tuple) and len(result) == 2:
        point, raw_text = result
    else:
        raw_text = str(result)
        response_json = extract_json_part(raw_text)
        point = -1
        if response_json is not None:
            points = json.loads(response_json, strict=False).get("points", [])
...
def process_task(
    ...
    """
    Build a consecutive grid and ask the configured VLM for the exact event
    or ends, and return the chosen frame index.

    Steps:

    1. Choose candidate frames using select_top_n_frames_from_json and
       filtering helpers. When *pinch_folder* is provided, each relative
       minimum pinch-distance frame and its immediate neighbors ``m-1`` and
       ``m+1`` are unioned with the speed keyframes (deduped, no weighting).
    ...
    credentials : dict
        Unused legacy credentials argument.
    video_path : str
        Input video.
    action : str
    ...
    speed_folder : str
        Folder containing metrics/*_hand_speed.json files.
    ...
    pinch_folder : str | None, optional
        Folder containing metrics/*_pinch_distance.json files. When provided, pinch keyframes
        are merged with the speed keyframes in the candidate pool.
    trial_idx : int | None, optional
        Trial number used only for naming debug grid outputs.
    stage : str | None, optional
        Human-readable localization stage used only for naming debug grid
        outputs.
    ...
    """
    validate_sampling_mode(mode)
    prompt_message = build_exact_localization_prompt(search_anchor, action)
    video_name = os.path.splitext(os.path.basename(video_path))[0]
    speed_json_path = (
        get_json_path(video_name, base_dir=speed_folder)
        if mode in ("speed", "speed_pinch")
        else None
    )
    pinch_json_path = (
        get_pinch_json_path(video_name, base_dir=pinch_folder)
        if mode in ("pinch", "speed_pinch") and pinch_folder
        else None
    )
    ...
    used_frame_indices, window = build_consecutive_candidate_grid(
        mode_data["candidates"],
        grid_size,
        total_frames,
    ...
    trial_folder = (
        f"trial_{trial_idx:02d}" if trial_idx is not None else "trial_unknown"
    )
    grid_dir = os.path.join(
        speed_folder, "localization_grids", trial_folder
    )
    os.makedirs(grid_dir, exist_ok=True)
    input_grid_path = os.path.join(
        grid_dir,
        f"{video_name}_{stage_label}_{search_anchor}_input.png",
    )
    if not cv2.imwrite(input_grid_path, image):
        raise IOError(f"failed to save localization grid: {input_grid_path}")
    trace = {
        "stage": stage or _default_localization_stage(
            search_anchor, frame_index, flag
        ),
        "mode": mode,
        "candidates": list(mode_data["candidates"]),
        "cue_candidates": list(mode_data["cue_candidates"]),
        "candidate_sources": mode_data["candidate_sources"],
        "total_pool": list(mode_data["total_pool"]),
        "fallback_frames": list(mode_data["fallback_frames"]),
        ...
        "flag": flag,
        "selected_frame": None,
        "saved_grid_path": input_grid_path,
        "selected_grid_path": None,
        "window": window,
        "grid_topology": "consecutive",
        "midpoint_restriction": False,
        "prompt": prompt_message,
        "signal_reads": mode_data["signal_reads"],
    }
    ...
    if description is None or str(description).strip() == "-1":
        trace["parse_error"] = description is None
        return None
    try:
        cell = int(description)
    except (TypeError, ValueError):
        trace["parse_error"] = True
        return None
    if not 1 <= cell <= len(used_frame_indices):
        trace["parse_error"] = True
        return None
    selected_frame_index = used_frame_indices[cell - 1]
    trace["selected_frame"] = selected_frame_index
    trace["parse_error"] = False
    ...
    selected_grid_path = os.path.join(
        grid_dir,
        f"{video_name}_{stage_label}_{search_anchor}_selected_"
        f"f{selected_frame_index:04d}.png",
    )
    if not cv2.imwrite(selected_grid_path, annotated_image):
        raise IOError(f"failed to save selected grid: {selected_grid_path}")
    trace["selected_grid_path"] = selected_grid_path
    print(
        f"Trial-2-final mode={mode} consecutive grid in {video_name}: "
        f"{used_frame_indices}; selected={selected_frame_index}"
    )
...
def select_frames_near_average(filter_indices, grid_size, total_frames, invalid_list, min_index=None):
    ...
    """
    frames, _ = build_consecutive_candidate_grid(
        filter_indices,
        grid_size,
        total_frames,
        min_index=min_index,
...
def select_and_filter_keyframes_with_anchor(selected_indices, total_indices, grid_size, search_anchor, video_path):
    """
    Legacy Trial-2 midpoint helper; Trial-2-final never calls this function.
...
def determine_by_state(credentials, video_path, action, grid_size, total_frames, frame_index, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, stage=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend=None):
    """
    Verify if a chosen frame truly shows contact/separation; if not, try again.

    The function sends one frame to the persistent Transformers backend.
    ...
    Args:
        credentials (dict): Unused legacy argument.
        video_path (str): Video file.
    ...
    if search_anchor == "start":
        prompt_message = (
            "This image is a candidate exact contact frame. Is a stable grasp "
            "of the object visibly established in this frame? Answer 1 for "
            "yes or 0 for no."
        )
    else:
        prompt_message = (
            "This image is a candidate exact separation frame. Is visible "
            "release of the object occurring in this frame? Answer 1 for yes "
            "or 0 for no."
        )
    ...
    grid_path = os.path.join(
        grid_dir, f"{video_name}_{_safe_filename_part(stage_name)}_input.png"
    )
    if not cv2.imwrite(grid_path, image):
        raise IOError(f"failed to save visual-feedback image: {grid_path}")
    trace = {
        "stage": stage_name,
        "mode": mode,
        "candidates": frame_indices,
        "cue_candidates": frame_indices,
        "candidate_sources": {"speed": [], "pinch": []},
        "total_pool": frame_indices,
        "fallback_frames": [],
        ...
        "saved_grid_path": grid_path,
        "selected_grid_path": None,
        "window": {
            "topology": "single_frame_visual_feedback",
            "midpoint_restriction": False,
        },
        "grid_topology": "single_frame_visual_feedback",
        "midpoint_restriction": False,
        "prompt": prompt_message,
        "signal_reads": {
            "speed": mode in ("speed", "speed_pinch"),
    ...
    accepted = answer.startswith("1")
    trace["binary_decision"] = 1 if accepted else 0
    trace["auto_accepted_near_end"] = bool(frame_index > total_frames - 5)
    if accepted or frame_index > total_frames - 5:
        return frame_index
    return process_task(
        credentials,
        video_path,
...
def determine_by_speed(credentials, video_path, action, grid_size, total_frames, frame_index, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, stage=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend=None):
    """
...
def scene_understanding(credentials, frame, prompt_message, flag=None, task="contact"):
    """
    Send an image and prompt through the shared Transformers NF4 backend.

    The *task* argument selects the already-attached contact or separation
    adapter. ``credentials`` is retained only for call compatibility.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--credentials", type=str, required=True, help="Unused legacy credentials path")
