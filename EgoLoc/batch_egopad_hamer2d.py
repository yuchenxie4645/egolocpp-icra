...
Outputs (relative to `--output_root`, default `/home/data_labeling/data/EgoLoc_samples/EgoPAT3D`):
...
def main():
    ...
    parser.add_argument("--trial", default="trial3",
                        choices=["trial0", "trial1", "trial2", "trial3"],
                        help="Pipeline module to drive. trial0 is speed-only; "
                             "trial1-3 add the pinch step.")
    parser.add_argument("--video_dir", default="/home/data_labeling/data/EgoLoc_samples/EgoPAT3D/EgoPAT3D/video")
    parser.add_argument("--output_root", default="/home/data_labeling/data/EgoLoc_samples/EgoPAT3D",
    ...
    parser.add_argument("--end", type=int, default=112)
    ...
    pipeline = importlib.import_module(args.trial)
    setup_hamer_cache = pipeline.setup_hamer_cache
    build_detector = pipeline.build_detector
    build_vitpose = pipeline.build_vitpose
    convert_video = pipeline.convert_video
    extract_2d_speed_and_visualize = pipeline.extract_2d_speed_and_visualize
    visualize_frame = pipeline.visualize_frame
    extract_pinch_distance_and_visualize = getattr(
        pipeline, "extract_pinch_distance_and_visualize", None)
    convert_video_supports_pinch = (
        "pinch_folder" in inspect.signature(convert_video).parameters)
    pinch_enabled = extract_pinch_distance_and_visualize is not None and convert_video_supports_pinch
    print(f"[trial={args.trial}] pinch step "
          f"{'enabled' if pinch_enabled else 'disabled (speed-only)'}")
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for video in videos:
            ...
            if extract_pinch_distance_and_visualize is not None:
                extract_pinch_distance_and_visualize(
                    str(video), str(out_dir), detector, vitpose, str(debug_dir)
                )

            convert_kwargs = {"repeat_times": args.repeat_times}
            if convert_video_supports_pinch:
                convert_kwargs["pinch_folder"] = str(out_dir)
            contact, separation = convert_video(
                str(video), args.action, creds, args.grid_size,
                str(out_dir), args.max_feedbacks, **convert_kwargs,
            )
