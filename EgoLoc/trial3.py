...
def _relative_minimum_frame_indices(detected_items):
    """
    Return frame indices that are local minima in the detected pinch-distance
    curve. Frames without hand detections should already be excluded.
    """
    detected_items = sorted((int(frame), float(distance)) for frame, distance in detected_items)
    if not detected_items:
        return []
    if len(detected_items) <= 2:
        frame_idx, _ = min(detected_items, key=lambda item: item[1])
        return [frame_idx]

    minima = []
    for i in range(1, len(detected_items) - 1):
        prev_distance = detected_items[i - 1][1]
        frame_idx, distance = detected_items[i]
        next_distance = detected_items[i + 1][1]
        if distance <= prev_distance and distance <= next_distance and (
            distance < prev_distance or distance < next_distance
        ):
            minima.append(frame_idx)
...
def _expand_minima_with_neighbors(minima, total_frames):
    """Return sorted unique frames from each local minimum's [m-1, m, m+1]."""
    expanded = []
...
def extract_pinch_distance_and_visualize(video_path, output_dir, detector, vitpose, debug_dir):
    """
    ...
    per-frame distance plus a curve plot. Annotated debug PNGs are written only
    for frames where a hand is detected; frames without detections are still
    ...
    relative_minimum_frames = _relative_minimum_frame_indices(detected_items)
    relative_minimum_neighborhood_frames = _expand_minima_with_neighbors(
        relative_minimum_frames, total_frames
    )
    for frame_idx in relative_minimum_frames:
        pinch_frames[str(frame_idx)]["is_relative_minimum"] = True
    for frame_idx in relative_minimum_neighborhood_frames:
        frame_record = pinch_frames.setdefault(
            str(frame_idx),
            {
                "detected": False,
        ...
        frame_record["is_relative_minimum_neighbor"] = frame_idx not in relative_minimum_frames
    ...
    pinch_json = {
        ...
        "candidate_selection": {
            ...
            "relative_minimum_frames": relative_minimum_frames,
            "relative_minimum_neighborhood_frames": relative_minimum_neighborhood_frames,
        },
        "frames": pinch_frames,
    }
    ...
    if detected_items:
        ...
        if relative_minimum_frames:
            plt.scatter(
                relative_minimum_frames, minima_y,
                color='g', s=24, zorder=3,
                label='relative-minimum pinch frames',
            )
        neighbor_only_x = [
            frame_idx
            for frame_idx in relative_minimum_neighborhood_frames
            if frame_idx not in relative_minimum_frames
            and pinch_frames[str(frame_idx)]["detected"]
        ]
        neighbor_only_y = [
            pinch_frames[str(frame_idx)]["pinch_distance_px"]
            for frame_idx in neighbor_only_x
        ]
        if neighbor_only_x:
            plt.scatter(
                neighbor_only_x, neighbor_only_y,
                color='orange', s=18, zorder=3,
                label='m-1/m+1 sampled neighbors',
            )
        plt.xlabel('Frame Index')
...
def feedback_contact(credentials, video_path, action, grid_size, total_frames, frame_start, max_feedbacks, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend="vllm"):
    """
    ...
    Args:
        ...
        pinch_folder (str | None): Folder holding metrics/*_pinch_distance.json files. When
            ...
            the candidate pool.
        mode (str): Sampling mode ("speed", "pinch" or "speed_pinch").
            In pinch mode only the visual feedback runs — the speed check
            (*determine_by_speed*) is skipped entirely.
        trial_seed (int | None): Optional seed forwarded to VLM requests.
    ...
    while feedback_count < max_feedbacks:
        corrected_frame = determine_by_state(
            ...
            mode=mode, trial_seed=trial_seed, trace_log=trace_log,
...
def feedback_separation(credentials, video_path, action, grid_size, total_frames, frame_end, max_feedbacks, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend="vllm"):
    """
...
def convert_video(video_path, action, credentials, grid_size, speed_folder, max_feedbacks, repeat_times=5, pinch_folder=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend="vllm"):
    """
    ...
    Args:
        ...
        pinch_folder (str | None): Folder containing metrics/*_pinch_distance.json files. When
            provided, pinch keyframes are merged with the speed keyframes in
            the candidate pool. Ignored entirely in speed mode.
        mode (str): Sampling mode ("speed", "pinch" or "speed_pinch");
            forwarded to every localization and feedback call.
        trial_seed (int | None): Optional seed forwarded to VLM requests so
            trials can be reproduced deterministically.
...
def select_pinch_keyframes(pinch_json_path, percentile=None):
    """
    Select pinch keyframes from relative minima plus their immediate neighbors.

    The current JSON format stores one record per video frame and marks
    ``is_relative_minimum`` for detected frames that are local minima, then
    expands each minimum ``m`` to ``[m-1, m, m+1]`` for localization-grid
    sampling. Legacy numeric JSON files are still supported by computing local
    minima and expanding them at read time. ``percentile`` is accepted only for
    old call-site compatibility and is intentionally ignored.

    Args:
        pinch_json_path (str): Path to ``*_pinch_distance.json``.
        percentile (int | None): Deprecated and ignored.

    Returns:
        list[int]: Sorted relative-minimum neighborhood frame indices. Empty
        list if the JSON is missing, empty, or has no detections.
    """
    ...
    if not data:
        return []
    if "candidate_selection" in data:
        frames = data["candidate_selection"].get("relative_minimum_neighborhood_frames")
        if frames is None:
            minima = data["candidate_selection"].get("relative_minimum_frames", [])
            frames = _expand_minima_with_neighbors(minima, int(data.get("total_frames", 0)))
    ...
    if "frames" in data:
        frames = [
            ...
            and (record.get("is_relative_minimum") or record.get("is_relative_minimum_neighbor"))
        ]
        if frames:
        ...
        minima = [
            int(frame_idx)
            for frame_idx, record in data["frames"].items()
            if isinstance(record, dict) and record.get("is_relative_minimum")
        ]
        if minima:
            return _expand_minima_with_neighbors(minima, int(data.get("total_frames", 0)))
    detected_items = _detected_pinch_items_from_json(data)
    minima = _relative_minimum_frame_indices(detected_items)
    total_frames = int(data.get("total_frames", 0)) if isinstance(data, dict) else 0
    if total_frames <= 0 and detected_items:
        total_frames = max(frame_idx for frame_idx, _ in detected_items) + 1
    return _expand_minima_with_neighbors(minima, total_frames)
...
SamplingMode = Literal["speed", "pinch", "speed_pinch"]

_VALID_SAMPLING_MODES = ("speed", "pinch", "speed_pinch")


def validate_sampling_mode(mode):
    """
    Return *mode* if it is a supported sampling mode, else raise ValueError.

    Args:
        mode (str): Candidate sampling mode.

    Returns:
        str: The validated mode, unchanged.

    Raises:
        ValueError: If *mode* is not one of "speed", "pinch",
            "speed_pinch".
    """
    if mode not in _VALID_SAMPLING_MODES:
        raise ValueError(
            f"Unsupported sampling mode {mode!r}; "
            f"expected one of {list(_VALID_SAMPLING_MODES)}"
        )
    return mode


def derive_seed(episode, task, trial):
    """
    Deterministic seed for one ablation trial.

    The seed depends only on ``(episode, task, trial)`` and never on the
    sampling mode, so every mode of the same trial runs with the same seed
    (paired comparison). SHA-256 is used instead of the builtin ``hash``
    because the latter is randomized per process.

    Args:
        episode: Episode identifier (any value with a readable str()).
        task (str): Task alias, e.g. "contact" or "separation".
        trial (int | str): Trial number within the episode.

    Returns:
        int: Non-negative 32-bit seed.
    """
    digest = hashlib.sha256(f"{episode}|{task}|{trial}".encode("utf-8")).hexdigest()
...
def build_mode_candidates(
    ...
    """
    Central candidate construction for every sampling mode.

    This is the only place where the signal JSONs are opened for candidate
    selection. ``process_task`` consumes its output instead of selecting
    signals itself, so each mode fully controls which signal files are read.

    Mode semantics:

        - "speed": candidates are the *n_candidates* lowest non-zero
          hand-speed frames from *speed_json_path*; flag/frame_index/
          receive_flag semantics are exactly those of
          *select_top_n_frames_from_json*, including the
          ``(invalid_list, candidates)`` pairing when *receive_flag* is set.
          total_pool is the speed-ranked list of valid frames. The pinch
          JSON is never opened.
        - "pinch": candidates are the pinch relative-minima ±1-neighbor
          frames from *pinch_json_path*; total_pool is the chronological
          ascending frame range. The speed JSON is never opened. For
          flag == "feedback" the positional exclusion is already handled by
          the grid builder's *min_index*, so the invalid_list is empty; for
          flag == "fault" the invalid_list is ``[frame_index]`` (positional,
          signal-neutral).
        - "speed_pinch": the historical behavior — speed candidates merged
          with the pinch candidates via *_merge_speed_and_pinch* and a
          speed-ranked total_pool.

    Args:
        mode (str): "speed", "pinch" or "speed_pinch".
        speed_json_path (str | None): Path to the hand-speed JSON. May be
            None in pinch mode (it is never read there).
        pinch_json_path (str | None): Path to the pinch-distance JSON. May
            be None in speed mode (it is never read there).
        n_candidates (int): Number of speed candidates to select (pinch
            candidates are not truncated; the grid builder owns the budget).
        frame_index (int | None): Reference frame index or speed threshold.
        flag (str | None): Behavior switch ("feedback", "speed", "fault").
        receive_flag (str | None): If set, also return the discarded frames
            as invalid_list (speed modes only, requires frame_index).
        total_frames (int | None): Total frame count of the video.

    Returns:
        dict: Keys ``mode``, ``candidates``, ``total_pool``,
        ``invalid_list`` and ``coverage`` (counts of speed/pinch/neutral
        sources among the evidence candidates).
    """
    validate_sampling_mode(mode)
    result = {
        "mode": mode,
        ...
        "total_pool": [],
        "invalid_list": [],
        "coverage": {"speed": 0, "pinch": 0, "neutral": 0},
        "candidate_sources": {"speed": [], "pinch": []},
    }

    if mode == "pinch":
        if pinch_json_path is not None:
            pinch_candidates = select_pinch_keyframes(pinch_json_path)
        else:
            pinch_candidates = []
        result["candidates"] = list(pinch_candidates)
        if total_frames is not None and int(total_frames) > 0:
            result["total_pool"] = list(range(int(total_frames)))
        if flag == "fault" and frame_index is not None:
            result["invalid_list"] = [int(frame_index)]
        result["coverage"] = {
            "speed": 0,
            "pinch": len(pinch_candidates),
            "neutral": 0,
        }
        result["candidate_sources"]["pinch"] = list(pinch_candidates)
        return result

    # "speed" and "speed_pinch" share the speed-signal selection below.
    if frame_index is None:
        speed_candidates = select_top_n_frames_from_json(speed_json_path, n_candidates)
        invalid_list = []
        total_pool = (
            list(select_top_n_frames_from_json(speed_json_path, total_frames))
            if total_frames is not None
            else []
        )
    else:
        if receive_flag is not None:
            invalid_list, speed_candidates = select_top_n_frames_from_json(
                speed_json_path, n_candidates, frame_index, flag, receive_flag=receive_flag
            )
            invalid_list = list(invalid_list)
        else:
            speed_candidates = select_top_n_frames_from_json(
                speed_json_path, n_candidates, frame_index, flag
            )
            invalid_list = []
        total_pool = list(
            select_top_n_frames_from_json(speed_json_path, total_frames, frame_index, flag)
        )

    if mode == "speed":
        result["candidates"] = list(speed_candidates)
        result["invalid_list"] = invalid_list
        result["total_pool"] = total_pool
        result["coverage"] = {
            "speed": len(speed_candidates),
            "pinch": 0,
            "neutral": 0,
        }
        result["candidate_sources"]["speed"] = list(speed_candidates)
        return result

    # mode == "speed_pinch": merge with the pinch candidates as before.
    if pinch_json_path is not None:
        pinch_candidates = select_pinch_keyframes(pinch_json_path)
    else:
        pinch_candidates = []
    result["candidates"] = _merge_speed_and_pinch(speed_candidates, pinch_candidates)
    result["invalid_list"] = invalid_list
    result["total_pool"] = total_pool
    result["coverage"] = {
        "speed": len(speed_candidates),
    ...
    result["candidate_sources"] = {
        "speed": list(speed_candidates),
        "pinch": list(pinch_candidates),
    }
    return result


def _frame_in_anchor_region(frame_idx, total_frames, search_anchor):
    """
    Preserve the old start/end anchor behavior:
      - start: prefer first half
      - end: prefer second half

    This is used as a first-pass filter. The direct-grid builder can still
    relax this later if it needs more frames to fill the 3x3 grid.
    """
    midpoint = total_frames // 2

    if search_anchor == "start":
        return frame_idx < midpoint
    if search_anchor == "end":
        return frame_idx >= midpoint
...
def _clean_int_list(values):
    """Convert a list-like object to clean integer frame indices."""
    clean = []
    if values is None:
        return clean
...
def build_direct_candidate_grid(
    selected_indices,
    total_indices,
    grid_size,
    total_frames,
    ...
    """
    Build the VLM grid directly from selected speed/pinch candidates.

    This replaces the old average-centered grid behavior.

    Old behavior:
        candidates -> mean(candidates) -> sequential window around mean

    New behavior:
        candidates -> direct grid -> fill remaining slots if needed -> sort chronologically

    Args:
        selected_indices (list[int]):
            Main evidence frames. In your pipeline this is the merged union of
            speed-selected frames and pinch-selected frames.

        total_indices (list[int]):
            Backup frames, usually all valid speed frames sorted from lowest
            speed to highest speed.

        grid_size (int):
            Grid side length. For 3x3, this is 3.
        ...
        invalid_list (list[int] | None):
            Frames that should be avoided during feedback.

        min_index (int | None):
            Optional minimum frame index. Used in visual feedback where the
            next grid should search after the previous selected frame.

    Returns:
        list[int]:
            Chronologically sorted frame indices for the VLM grid.
    """
    target_count = grid_size ** 2
    selected_indices = _clean_int_list(selected_indices)
    total_indices = _clean_int_list(total_indices)
    invalid_set = set(_clean_int_list(invalid_list))
    ...
    def basic_allowed(frame_idx, honor_invalid=True, honor_min=True):
        if frame_idx < 0 or frame_idx >= total_frames:
            return False

        if honor_invalid and frame_idx in invalid_set:
            return False

        if honor_min and min_frame is not None and frame_idx <= min_frame:
            return False

        return True

    def add_frame(frame_idx, require_anchor=True, honor_invalid=True, honor_min=True):
        try:
            frame_idx = int(frame_idx)
        except (TypeError, ValueError):
            return False

        if not basic_allowed(frame_idx, honor_invalid=honor_invalid, honor_min=honor_min):
            return False

        if require_anchor and not _frame_in_anchor_region(frame_idx, total_frames, search_anchor):
            return False
    ...
    # ------------------------------------------------------------------
    # 1. Add the actual evidence frames first:
    #    speed candidates + pinch candidates, already merged upstream.
    #    This is the core change: keep the actual candidate frames.
    # ------------------------------------------------------------------
    evidence_seed = []
    ...
    for frame_idx in selected_indices:
        if len(final_frames) >= target_count:
            break
        added = add_frame(frame_idx, require_anchor=True)
        if added:
            evidence_seed.append(int(frame_idx))

    # If the old hard anchor split removed everything, relax anchor once.
    # This prevents empty/black grids for short videos or boundary events.
    if len(final_frames) == 0:
        for frame_idx in selected_indices:
            if len(final_frames) >= target_count:
                break
            added = add_frame(frame_idx, require_anchor=False)
            if added:
                evidence_seed.append(int(frame_idx))

    # ------------------------------------------------------------------
    # 2. Add the average as ONE optional candidate, not as the grid center.
    #    This preserves your intuition of "customly added average" without
    #    letting average dominate the whole 3x3 grid.
    # ------------------------------------------------------------------
    if len(final_frames) < target_count and evidence_seed:
        avg_idx = int(round(float(np.mean(evidence_seed))))
        add_frame(avg_idx, require_anchor=False)

    # ------------------------------------------------------------------
    # 3. Fill remaining slots using backup low-speed frames.
    #    First respect the start/end anchor, then relax it if still short.
    # ------------------------------------------------------------------
    for frame_idx in total_indices:
        if len(final_frames) >= target_count:
            break
        add_frame(frame_idx, require_anchor=True)

    for frame_idx in total_indices:
        if len(final_frames) >= target_count:
            break
        add_frame(frame_idx, require_anchor=False)

    # ------------------------------------------------------------------
    # 4. Add local temporal context around the selected evidence frames.
    #    This replaces the old "sequential around average" behavior with
    #    local context around actual candidate frames.
    # ------------------------------------------------------------------
    context_centers = sorted(set(evidence_seed if evidence_seed else final_frames))

    for radius in range(1, total_frames + 1):
        if len(final_frames) >= target_count:
            break
        ...
        for center in context_centers:
            if len(final_frames) >= target_count:
                break
            add_frame(center - radius, require_anchor=False)
    ...
    # ------------------------------------------------------------------
    # 5. Final fallback: fill from the video chronologically.
    #    This should rarely be needed, but avoids producing empty grids.
    # ------------------------------------------------------------------
    ...
    # Last-resort fallback: ignore invalid/min constraints only to avoid crash.
    if len(final_frames) == 0:
        for frame_idx in range(total_frames):
            if len(final_frames) >= target_count:
                break
    ...
    # Important: display chronologically so the VLM sees temporal progression.
    return sorted(final_frames[:target_count])
...
def _publish_request_trace(trace_out, trace_log, request_trace):
    """Publish independent copies of one completed model-request trace."""
    if trace_out is not None:
        trace_out.clear()
        trace_out.update(deepcopy(request_trace))
    if trace_log is not None:
        trace_log.append(deepcopy(request_trace))
...
def process_task(
    ...
    """
    Build a grid image, ask the VLM to pick the frame where the action starts
    or ends, and return the chosen frame index.

    Change 4 version:
        The grid is now built directly from the merged speed/pinch candidates.
        It no longer computes the average candidate frame and then builds a
        sequential 3x3 window around that average.

    Preserved:
        - speed candidate selection
        - pinch candidate selection
        - m-1, m, m+1 pinch neighborhood from select_pinch_keyframes()
        - speed + pinch union
    ...
    Sampling modes (*mode*):
        - "speed": top-4 lowest hand-speed candidates, speed-ranked fallback
          pool; the pinch JSON is never opened.
        - "pinch": pinch relative-minima ±1-neighborhood candidates,
          chronological fallback pool; the speed JSON is never opened and
          no speed check is performed during feedback.
        - "speed_pinch": historical behavior — speed ∪ pinch candidates with
          a speed-ranked fallback pool (default; identical to the
    ...
    Args:
        ...
        video_path (str): Path to the input video.
        action (str): Action description shown to the model.
        grid_size (int): Grid side length for the candidate grid.
        total_frames (int): Total frame count of the video.
        search_anchor (str): "start" (contact) or "end" (separation).
        speed_folder (str): Folder holding the speed JSONs (also receives
            the localization grid debug images in every mode).
        frame_index (int | None): Reference frame for feedback stages.
        flag (str | None): "feedback", "speed", "fault" or None.
        pinch_folder (str | None): Folder holding pinch JSONs; ignored in
            speed mode.
        trial_idx (int | None): Trial number used for debug output naming.
        stage (str | None): Stage label for debug output naming.
        mode (str): Sampling mode; see *build_mode_candidates*.
        trial_seed (int | None): Optional seed forwarded to the VLM request.
        trace_out (dict | None): When given, filled with the selection and
            request trace (stage, mode, candidates, total_pool,
            used_frame_indices, raw_response, latency_s, backend_used, seed,
            flag). Only the dict passed in is written.

    Returns:
        int | None: Chosen frame index, or None if not determined.
    """
    validate_sampling_mode(mode)
    prompt_start = (
    ...
    if mode == "speed":
        ...
        pinch_json_path = None
    elif mode == "pinch":
        json_path = None
        pinch_json_path = (
            get_pinch_json_path(video_name, base_dir=pinch_folder) if pinch_folder else None
        )
    else:
        json_path = get_json_path(video_name, base_dir=speed_folder)
        pinch_json_path = (
    ...
    mode_data = build_mode_candidates(
        mode,
        json_path,
        pinch_json_path,
        n_candidates=4,
        ...
        flag=flag,
        receive_flag="right" if frame_index is not None else None,
        total_frames=total_frames,
    )
    selected_indices = mode_data["candidates"]
    total_indices = mode_data["total_pool"]
    invalid_list = mode_data["invalid_list"]

    used_frame_indices = build_direct_candidate_grid(
        ...
        min_index=frame_index if flag == "feedback" else None,
    )
    ...
    print(f"Sampling mode '{mode}' candidates in {video_name}: {selected_indices}")
    print(f"Direct candidate grid frames in {video_name}: {used_frame_indices}")

    request_trace = {
        "stage": stage or _default_localization_stage(search_anchor, frame_index, flag),
        "mode": mode,
        "candidates": list(selected_indices),
        "candidate_sources": deepcopy(mode_data.get("candidate_sources", {})),
        "total_pool": list(total_indices),
        "used_frame_indices": list(used_frame_indices),
        "flag": flag,
        "selected_frame": None,
        "saved_grid_path": None,
    }
    ...
    try:
        description, reason = vlm_request(
            image,
            prompt_message,
            task,
            ...
            seed=trial_seed,
            flag=None,
            trace_out=request_trace,
        )
    except Exception as exc:
        request_trace["error_class"] = type(exc).__name__
        request_trace["error_message"] = str(exc)
        request_trace["flag"] = flag
        _publish_request_trace(trace_out, trace_log, request_trace)
        raise
    # The trace flag describes the localization stage, not the VLM request.
    request_trace["flag"] = flag
    ...
    grid_dir = os.path.join(
        speed_folder, "localization_grids", _safe_filename_part(mode), trial_folder
    )
    ...
    request_trace["selected_frame"] = int(selected_frame_index)
    request_trace["saved_grid_path"] = debug_grid_path
    _publish_request_trace(trace_out, trace_log, request_trace)
...
def determine_by_state(credentials, video_path, action, grid_size, total_frames, frame_index, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, stage=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend="vllm"):
    """
    Verify if a chosen frame truly shows contact/separation; if not, try again.

    The function sends the single frame to the VLM asking a yes/no question.
    ...
    Args:
        ...
        mode (str): Sampling mode forwarded to *process_task*.
        trial_seed (int | None): Optional seed forwarded to the VLM request
            and to any resampling *process_task* call.
    ...
    task = "contact" if search_anchor == "start" else "separation"
    stage_name = stage or f"{task}_visual_feedback"
    trial_folder = f"trial_{trial_idx:02d}" if trial_idx is not None else "trial_unknown"
    grid_dir = os.path.join(
        speed_folder, "localization_grids", _safe_filename_part(mode), trial_folder
    )
    ...
    debug_grid_path = os.path.join(
        grid_dir,
        f"{video_name}_{_safe_filename_part(stage_name)}_{search_anchor}_check_f{int(frame_index):04d}.png",
    )
    cv2.imwrite(debug_grid_path, image)
    request_trace = {
        "stage": stage_name,
        "mode": mode,
        "candidates": [int(frame_index)],
        "candidate_sources": {"speed": [], "pinch": []},
        "total_pool": [int(frame_index)],
        "used_frame_indices": [int(frame_index)],
        "flag": flag,
        "selected_frame": None,
        "saved_grid_path": debug_grid_path,
    }
    try:
        result = vlm_request(
            image, prompt_message, task, backend=inference_backend,
            seed=trial_seed, flag=flag, trace_out=request_trace,
        )
    except Exception as exc:
        request_trace["error_class"] = type(exc).__name__
        request_trace["error_message"] = str(exc)
        _publish_request_trace(None, trace_log, request_trace)
        raise
    accepted = str(result).strip() == "1" or frame_index > total_frames - 5
    if accepted:
        request_trace["selected_frame"] = int(frame_index)
    _publish_request_trace(None, trace_log, request_trace)
    if accepted:
        return frame_index
    else:
...
def determine_by_speed(credentials, video_path, action, grid_size, total_frames, frame_index, search_anchor, speed_folder, pinch_folder=None, trial_idx=None, stage=None, mode="speed_pinch", trial_seed=None, trace_log=None, inference_backend="vllm"):
    """
    ...
    Args are the same as *determine_by_state*, plus *mode* and *trial_seed*.
    In pinch mode this check never runs — the frame is accepted without
    reading the speed JSON — because the pinch signal carries no hand-speed
    information.
...
class _VLMTransportBadRequest(Exception):
    """The vLLM server rejected a request (HTTP 400)."""


def _build_vlm_prompt_messages(image_bgr, prompt_message):
    """Build the OpenAI Vision message list for one BGR image."""
    frame = image_resize_for_vlm(image_bgr)
...
def _request_vllm(image_bgr, prompt_message, task, seed):
    """Send one chat request to the local vLLM server and return raw text.

    Transport parameters and retry semantics are identical to the original
    *scene_understanding* implementation. Raises *_VLMTransportBadRequest*
    when the server answers HTTP 400.
    """
    import openai
    ...
    params = {
        "model": task,
        "messages": _build_vlm_prompt_messages(image_bgr, prompt_message),
        "max_tokens": 200,
        "temperature": 0.1,
        "top_p": 0.5,
    ...
    while True:
        ...
        try:
            ...
            raise _VLMTransportBadRequest(str(e)) from e
...
_TRANSFORMERS_BACKEND_SINGLETON = None


def _request_transformers(image_bgr, prompt_message, task, seed, flag=None):
    """Run one request through backend.py's TransformersBackend.

    The backend module lives next to this file under ablation-tests/ and is
    imported lazily so the heavy model stack is only loaded when the
    transformers backend is actually requested. The backend instance is
    cached per process so the model loads exactly once.
    """
    global _TRANSFORMERS_BACKEND_SINGLETON
    if _TRANSFORMERS_BACKEND_SINGLETON is None:
        backend_dir = os.path.join(ROOT, "ablation-tests")
        if backend_dir not in sys.path:
            sys.path.insert(0, backend_dir)
        from backend import TransformersBackend
        _TRANSFORMERS_BACKEND_SINGLETON = TransformersBackend()
    return _TRANSFORMERS_BACKEND_SINGLETON.generate(
        image_bgr, prompt_message, task, seed=seed, flag=flag
    )


def _parse_vlm_point(raw_text):
    """Extract the first point from a {"points": [...]} VLM answer; -1 if absent."""
    response_json = extract_json_part(raw_text)
    ...
    return json_dict['points'][0]


def vlm_request(image_bgr, prompt_message, task, backend="vllm", seed=None, flag=None, trace_out=None):
    """
    Send one VLM request through a pluggable backend and parse the reply.

    Args:
        image_bgr (np.ndarray): BGR image array.
        prompt_message (str): Instruction for the model.
        task (str): Adapter alias routed through the request ("contact" or
            "separation").
        backend (str): "vllm" (OpenAI-compatible local server, the historical
            transport) or "transformers" (in-process TransformersBackend from
            ablation-tests/backend.py).
        seed (int | None): Optional sampling seed; forwarded to the backend
            when supported.
        flag (str | None): If None, parse and return the first point; when
            not None, return the raw text answer.
        trace_out (dict | None): When given, filled with raw_response,
            latency_s, backend_used, seed, task and flag. Only the dict
            passed in is written.

    Returns:
        Tuple[int, str] | str | Tuple[None, None]:
            • flag None: (chosen_point, full_text_response); the point is -1
              when the answer has no parseable points.
            • flag set: the full text response.
            • On an HTTP 400 from the vLLM server: (None, None) — matching
              the historical behavior regardless of *flag*.
    """
    started = time.time()
    bad_request = False
    point = None
    if isinstance(backend, str) and backend == "vllm":
        try:
            raw_text = _request_vllm(image_bgr, prompt_message, task, seed)
        except _VLMTransportBadRequest:
            bad_request = True
            raw_text = None
        backend_used = "vllm"
    elif isinstance(backend, str) and backend == "transformers":
        generated = _request_transformers(
            image_bgr, prompt_message, task, seed, flag=flag
        )
        if flag is None:
            point, raw_text = generated
        else:
            raw_text = generated
        backend_used = "transformers-nf4"
    elif not isinstance(backend, str) and callable(getattr(backend, "generate", None)):
        ...
        if flag is None:
            if isinstance(generated, tuple) and len(generated) == 2:
                ...
                raw_text = generated
                point = _parse_vlm_point(raw_text)
        else:
            raw_text = generated[1] if isinstance(generated, tuple) else generated
        backend_used = str(
            getattr(backend, "name", None) or backend.__class__.__name__
        )
    else:
        raise ValueError(
            f"Unknown VLM backend {backend!r}; expected 'vllm', "
            "'transformers', or an object with generate()"
        )
    latency_s = time.time() - started

    if trace_out is not None:
        trace_out["raw_response"] = raw_text
        trace_out["latency_s"] = latency_s
        trace_out["backend_used"] = backend_used
        trace_out["seed"] = seed
        trace_out["task"] = task
        trace_out["flag"] = flag
...
def scene_understanding(credentials, frame, prompt_message, flag=None, task="contact"):
    ...
    Thin backward-compatible wrapper around *vlm_request* using the vLLM
    backend without a seed. The image is first resized and encoded to base64,
    then combined with *prompt_message* in the OpenAI Vision API format.
    Depending on whether *flag* is `None`, the function either expects a JSON
    answer containing `{ "points": [...] }` or returns the raw text.

    The *task* argument selects which LoRA adapter handles the request via the
    ``model`` field: ``"contact"`` or ``"separation"``. *credentials* is kept
    ...
    """
    return vlm_request(frame, prompt_message, task, backend="vllm", seed=None, flag=flag)
